...
bool checkshouldon() //check based on time if it should on
{
    ...
    if (rollover)
    { //jika rollover
        if (timeinfo.tm_hour > relayontime_hour)
            checkonhour = true; //periksa jam saja sebelum roll
        else if (timeinfo.tm_min > relayontime_min && timeinfo.tm_hour == relayontime_hour)
            checkonhour = true; //periksa jika jam sama dgn on time
        else if (timeinfo.tm_hour < relayofftime_hour)
            checkonhour = true; //periksa jam saja setelah roll
        else if (timeinfo.tm_min > relayofftime_min && timeinfo.tm_hour == relayontime_hour)
            checkonhour = true; //periksa jika jam sama dgn off time
        else
            checkonhour = false;
    }
    else
    {
        if (timeinfo.tm_hour > relayontime_hour && timeinfo.tm_hour < relayofftime_hour)
            checkonhour = true; //periksa jam saja
        else if (timeinfo.tm_min > relayontime_min && timeinfo.tm_hour == relayontime_hour)
            checkonhour = true; //periksa jika jam sama dgn on time
        else if (timeinfo.tm_min < relayofftime_min && timeinfo.tm_hour == relayofftime_hour)
            checkonhour = true; //periksa jika jam sama dgn off time
        else
            checkonhour = false;
    }
